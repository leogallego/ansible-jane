...
# --- Helpers ---
# Uses `android layout` for accurate Compose element discovery
...
no_crash() {
    refresh_layout
    ! grep -qi "has stopped\|keeps stopping\|isn't responding" "$LAYOUT_JSON" 2>/dev/null
}
...
wait_for_app() {
    ...
    while [ $retries -gt 0 ]; do
        if $ADB -s "$DEVICE" shell "dumpsys activity activities" 2>/dev/null | grep -qi "ResumedActivity.*$PKG\|$PKG.*MainActivity"; then
            return 0
        fi
        sleep 1
        retries=$((retries - 1))
...
# Force-stop and clear for clean state
...
# Check if we're on auth screen or main screen
...
# ============================================
# AUTH SCREEN TESTS (if on auth screen)
# ============================================
if [ "$AUTH_MODE" = true ]; then
    section "Auth Screen"

    if has_resource_id "field_url"; then
        pass "URL field present"
    else
        fail "URL field missing"
    fi

    if has_resource_id "field_token"; then
        pass "Token field present"
    else
        fail "Token field missing"
    fi

    if has_resource_id "field_alias"; then
        pass "Alias field present"
    else
        fail "Alias field missing"
    fi

    if has_resource_id "switch_self_signed"; then
        pass "Self-signed cert switch present"
    else
        fail "Self-signed cert switch missing"
    fi

    if has_resource_id "button_connect"; then
        pass "Connect button present"
    else
...
if [ "$AUTH_MODE" = false ]; then
    ...
    if no_crash; then
        ...
        fail "Dashboard — crash detected"
    fi

    if has_resource_id "list_dashboard"; then
        pass "Dashboard list rendered"
    else
        skip "Dashboard list (may be empty or loading)"
    fi

    if has_resource_id "button_settings"; then
        pass "Settings button present"
    else
        fail "Settings button missing"
    fi

    if has_resource_id "button_notifications"; then
        pass "Notifications button present"
    else
        fail "Notifications button missing"
    fi
    ...
    if no_crash; then
        ...
        fail "Templates — crash detected"
    fi

    # Job Templates segment (default)
    if has_resource_id "segment_job_templates"; then
        pass "Job Templates segment visible"
    else
        fail "Job Templates segment missing"
    fi

    if has_resource_id "segment_workflows"; then
        pass "Workflows segment visible"
    else
        fail "Workflows segment missing"
    fi

    screenshot "03_templates_jobs"

    # Switch to Workflows segment
    tap_resource_id "segment_workflows" && {
        sleep 2
        if no_crash; then
            pass "Workflows segment — no crash"
        else
            fail "Workflows segment — crash detected"
        fi
        screenshot "04_templates_workflows"
    } || skip "Workflows segment tap"

    # Switch back to Job Templates
    ...
    if no_crash; then
        ...
        fail "Infrastructure — crash detected"
    fi

    # Inventories segment (default)
    if has_resource_id "segment_inventories"; then
        pass "Inventories segment visible"
    else
        fail "Inventories segment missing"
    fi

    if has_resource_id "segment_hosts"; then
        pass "Hosts segment visible"
    else
        fail "Hosts segment missing"
    fi
    ...
    tap_resource_id "segment_hosts" && {
        ...
        if no_crash; then
            ...
            fail "Hosts segment — crash detected"
        fi
    ...
    if no_crash; then
        ...
        fail "Activity — crash detected"
    fi

    # Jobs segment (default)
    if has_resource_id "segment_jobs"; then
        pass "Jobs segment visible"
    else
        fail "Jobs segment missing"
    fi

    if has_resource_id "segment_schedules"; then
        pass "Schedules segment visible"
    else
        fail "Schedules segment missing"
    fi

    if has_resource_id "segment_eda"; then
        pass "EDA segment visible"
    else
        fail "EDA segment missing"
    fi
    ...
    tap_resource_id "segment_schedules" && {
        ...
        if no_crash; then
            ...
            fail "Schedules segment — crash detected"
        fi
    ...
    tap_resource_id "segment_eda" && {
        ...
        if no_crash; then
            ...
            fail "EDA segment — crash detected"
        fi
    ...
    if no_crash; then
        ...
        fail "Assistant — crash detected"
    fi

    if has_resource_id "field_assistant_input"; then
        pass "Chat input field present"
    else
        fail "Chat input field missing"
    fi

    if has_resource_id "button_send"; then
        pass "Send button present"
    else
        fail "Send button missing"
    fi

    # Check for provider chip
    if has_resource_id "chip_provider"; then
        pass "Provider switch chip visible"
    else
        skip "Provider switch chip (may need config)"
    fi
    ...
    if has_resource_id "button_clear_chat"; then
        pass "Clear chat button present"
    else
        skip "Clear chat button (may not show when empty)"
    fi

    # Test token usage indicator
    if has_resource_id "text_session_tokens"; then
        pass "Session tokens indicator present"
    else
        skip "Session tokens (shows after chat activity)"
    fi
    ...
    tap_resource_id "button_notifications" && {
        sleep 2
        if has_resource_id "sheet_notifications"; then
            pass "Notifications sheet opened"
        else
            skip "Notifications sheet (may use different rendering)"
        fi

        if has_resource_id "button_refresh_notifications"; then
            pass "Refresh notifications button present"
        else
            skip "Refresh notifications button"
        fi

        screenshot "11_notifications"

        # Close the sheet by pressing back
        $ADB -s "$DEVICE" shell input keyevent KEYCODE_BACK
        sleep 1
    ...
    if no_crash; then
        ...
        fail "Settings — crash detected"
    fi

    screenshot "12_settings_general"

    # General tab (default)
    if has_text "Backup & Restore"; then
        pass "Backup & Restore section visible"
    else
        skip "Backup & Restore (may need scroll)"
    fi
    ...
    if tap_resource_id "tab_instances"; then
        ...
        if no_crash; then
            ...
            fail "Instances tab — crash detected"
        fi

        if has_text "Add Instance"; then
            pass "Add Instance button visible"
        else
            skip "Add Instance button (may need scroll)"
        fi
    ...
    if tap_resource_id "tab_agent"; then
        ...
        if no_crash; then
            ...
            fail "Agent tab — crash detected"
        fi

        if has_text "LLM Provider"; then
            pass "LLM Provider section visible"
        else
            fail "LLM Provider section missing"
        fi
    ...
    if tap_resource_id "tab_tools"; then
        ...
        if no_crash; then
            ...
            fail "Tools tab — crash detected"
        fi

        if has_text "MCP Servers"; then
            pass "MCP Servers section visible"
        else
            fail "MCP Servers section missing"
        fi

        if has_resource_id "switch_mcp_enabled"; then
            pass "MCP enabled switch present"
        else
            fail "MCP enabled switch missing"
        fi
    ...
    if has_resource_id "button_back"; then
        ...
        $ADB -s "$DEVICE" shell input keyevent KEYCODE_BACK
        sleep 1
        pass "Back from settings (keyevent)"
    fi
    ...
    if has_text "Jobs"; then
        pass "Dashboard Jobs section present"
    else
        fail "Dashboard Jobs section missing"
    fi

    if has_text "Resources"; then
        pass "Dashboard Resources section present"
    else
        skip "Dashboard Resources section (may need scroll)"
    fi

    # Scroll down to verify more content renders
    scroll_down 600
    sleep 1
    ...
    if no_crash; then
        ...
        fail "Dashboard scroll — crash detected"
    fi

    if has_text "Job Activity"; then
        pass "Dashboard Job Activity chart visible after scroll"
    else
        skip "Dashboard Job Activity chart (may need more scroll)"
    fi
    ...
    if has_resource_id "list_templates"; then
        ...
        # Check if any template cards are visible (by looking for launch buttons)
    ...
    # Scroll the template list
    scroll_down 400
    sleep 1
    if no_crash; then
        pass "Template list scroll — no crash"
    else
        fail "Template list scroll — crash detected"
    fi
    scroll_up 400
    sleep 1
    ...
    # Find and tap a launch button (button_launch_{id})
...
" 2>/dev/null) || true
    ...
    if [ -n "$LAUNCH_FOUND" ]; then
        ...
        if no_crash; then
            ...
            fail "Template launch — crash detected"
        fi

        # Check if we navigated to job status screen
        refresh_layout
        if grep -q '"resource-id":"scaffold_detail"\|"text":"Job Status"' "$LAYOUT_JSON" 2>/dev/null; then
            pass "Job status screen opened"

            if has_resource_id "text_job_name"; then
                pass "Job name displayed"
            else
                skip "Job name (may still be loading)"
            fi

            if has_resource_id "badge_job_status"; then
                pass "Job status badge displayed"
            else
                skip "Job status badge (may still be loading)"
            fi
    ...
    # Check for workflow template items (exclude the segment button text itself)
...
" 2>/dev/null) || true
    ...
    if [ -n "$WORKFLOW_NAME" ]; then
        ...
        # Try tapping a workflow to see detail screen
        tap_text "$WORKFLOW_NAME" 2>/dev/null && {
            sleep 2
            if no_crash; then
                pass "Workflow detail screen — no crash"
            else
                fail "Workflow detail screen — crash detected"
            fi

            if has_resource_id "button_launch_workflow"; then
                pass "Workflow launch FAB present"
            else
                skip "Workflow launch FAB (may not be in Success state)"
            fi
    ...
    if has_resource_id "list_inventories"; then
        ...
        if no_crash; then
            ...
            fail "Inventories scroll — crash detected"
        fi
        scroll_up 400
    else
    ...
    if has_resource_id "list_hosts"; then
        ...
        if no_crash; then
            ...
            fail "Hosts scroll — crash detected"
        fi
        scroll_up 400
    else
    ...
    if has_resource_id "list_jobs"; then
        ...
        if no_crash; then
            ...
            fail "Jobs list scroll — crash detected"
        fi
        scroll_up 400
    else
    ...
    if has_resource_id "list_schedules"; then
        pass "Schedules list rendered"
    else
        skip "Schedules list (may be loading or empty)"
    fi
    ...
    if has_resource_id "list_eda_audit"; then
        pass "EDA audit list rendered"
    else
        skip "EDA audit list (may be loading or empty)"
    fi
    ...
    # Check if LLM is configured (provider chip shows a provider name)
    ...
    if [ "$LLM_CONFIGURED" = true ]; then
        ...
        if no_crash; then
            ...
            fail "Chat send — crash detected"
        fi

        # Check if response appeared (streaming indicator or message)
        refresh_layout
        if has_resource_id "list_chat_messages"; then
            pass "Chat message list active"
        else
            skip "Chat message list (rendering may differ)"
        fi
        ...
        if no_crash; then
            ...
            fail "Chat response — crash detected"
        fi

        # Verify token usage appeared
        if has_resource_id "text_session_tokens"; then
            pass "Token usage indicator visible after chat"
        else
            skip "Token usage indicator (may not show for this provider)"
        fi
        ...
        if has_resource_id "button_clear_chat"; then
            ...
            if no_crash; then
                ...
                fail "Clear chat — crash detected"
            fi
        else
...
" 2>/dev/null || echo "0")
    ...
    if [ "$INSTANCE_COUNT" -gt 0 ]; then
        pass "Instance cards rendered ($INSTANCE_COUNT found)"
    else
        skip "Instance cards (may use text-only rendering)"
    fi

    # Verify active pill
    if has_text "Active"; then
        pass "Active instance indicator visible"
    else
        skip "Active instance indicator"
    fi

    # Verify Add Instance button
    if has_resource_id "button_add_instance"; then
        pass "Add Instance button present"
    else
        fail "Add Instance button missing"
    fi

    screenshot "31_instances_management"

    # Test logout all dialog
    scroll_down 300
    ...
    if has_text "Theme"; then
        pass "Theme section visible"
    else
        skip "Theme section (may need scroll)"
    fi
    ...
    if has_text "Backup & Restore"; then
        pass "Backup & Restore visible after scroll"
    else
        skip "Backup & Restore (may need more scroll)"
    fi

    if has_resource_id "button_export_backup"; then
        pass "Export backup button present"
    else
        skip "Export backup button (may need scroll)"
    fi

    if has_resource_id "button_import_backup"; then
        pass "Import backup button present"
    else
        skip "Import backup button (may need scroll)"
    fi
    ...
    if has_any_text_matching "OpenRouter\|Ollama\|OpenAI\|Gemini\|Custom"; then
        pass "Provider cards visible"
    else
        skip "Provider cards (may need configuration)"
    fi

    if has_resource_id "button_clear_history"; then
        pass "Clear history button present"
    else
        skip "Clear history button (may need scroll)"
    fi
    ...
    if has_text "Local Tools"; then
        pass "Local Tools section visible"
    else
        skip "Local Tools (may need scroll)"
    fi
    ...
    if has_any_text_matching "Jobs\|Inventory\|Monitoring\|Users\|Security\|Configuration\|EDA\|Platform"; then
        pass "Tool categories visible"
    else
        skip "Tool categories (may need more scroll)"
    fi
    ...
    if tap_text "Jobs"; then
        ...
        if no_crash; then
            ...
            fail "Tool category expand — crash detected"
        fi
    ...
    for tab in nav_dashboard nav_templates nav_infrastructure nav_activity nav_assistant; do
        ...
        if no_crash; then
            ...
            fail "Crash on quick nav to $tab"
        fi
    done
